...
void main() {
  ...
  group('o2CellSpreadBound', () {
    test('covers the largest excursion in either series, padded', () {
      expect(
        o2CellSpreadBound([
          [0.0],
          [3.0],
    ...
    test('accounts for the range curve, which is never negative', () {
      // The range line can exceed every individual deviation, and must still
      // fit inside the axis.
      expect(
        o2CellSpreadBound(
          [
    ...
    test('holds a fixed full scale so thin always means the same thing', () {
      // A tight rig must not be rescaled until its ribbon fills the chart.
      expect(
        o2CellSpreadBound([
          [0.0],
          [0.2],
        ...
        kO2CellSpreadFullScaleMv,
      );
      expect(
        o2CellSpreadBound(const [], rangeCurve: [1.0]),
        kO2CellSpreadFullScaleMv,
      );
    });

    test('grows only when a dive genuinely exceeds full scale', () {
      expect(
        o2CellSpreadBound(const [], rangeCurve: [40.0]),
        greaterThan(kO2CellSpreadFullScaleMv),
      );
    });

    test('ignores nulls and handles no data at all', () {
      expect(
        o2CellSpreadBound([
          [null],
          [null],
      ...
      expect(o2CellSpreadBound(const []), kO2CellSpreadFullScaleMv);
    });

    test('full scale leaves a tight rig visibly thin', () {
      // A 2 mV spread is drawn at +/-1 mV, so the ribbon must occupy a small
      // fraction of the axis rather than a quarter of it.
      expect(1.0 / kO2CellSpreadFullScaleMv, lessThanOrEqualTo(0.15));
    });
  });
