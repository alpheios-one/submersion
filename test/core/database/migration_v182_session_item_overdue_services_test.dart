...
/// Pre-v182 pre_dive_session_items shape: no frozen overdue-services column.
const _preV182SessionItems = '''
...
void main() {
  test('v182 adds the overdue_services column, preserving rows', () async {
    final nativeDb = NativeDatabase.memory(
      setup: (rawDb) {
        rawDb.execute('PRAGMA user_version = 181');
        rawDb.execute(_preV182SessionItems);
        rawDb.execute(
  ...
  test('migration list includes v182 and schema is at least 182', () {
    expect(AppDatabase.currentSchemaVersion, greaterThanOrEqualTo(182));
    expect(AppDatabase.migrationVersions, contains(182));
  });

  test('v182 is idempotent when overdue_services already exists', () async {
