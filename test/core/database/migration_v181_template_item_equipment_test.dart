...
/// Pre-v181 pre_dive_checklist_template_items shape: no equipment link.
const _preV181TemplateItems = '''
...
void main() {
  test('v181 adds the equipment_id column, preserving rows', () async {
    final nativeDb = NativeDatabase.memory(
      setup: (rawDb) {
        rawDb.execute('PRAGMA user_version = 180');
        rawDb.execute(_preV181TemplateItems);
        rawDb.execute(
  ...
  test('migration list includes v181 and schema is at least 181', () {
    expect(AppDatabase.currentSchemaVersion, greaterThanOrEqualTo(181));
    expect(AppDatabase.migrationVersions, contains(181));
  });

  test('v181 is idempotent when equipment_id already exists', () async {
